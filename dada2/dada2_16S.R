...
work_dir <- "/scratch1/choerstm/Kerguelen/16S/20200427_dada2_quality"
...
#PREFILTERING
filterAndTrim(fnFs.raw,fnFs.preFilt,fnRs.raw,fnRs.preFilt,truncQ=2,minQ=2,minLen=50,maxN=0,multithread = 10)


#IDENTIFY PRIMER
FWD_PRIMER="GTGCCAGCMGCCGCGGTAA" #515F-Y
REV_PRIMER="GGACTACHVGGGTWTCTAAT" #926R
...
dadaFWDs <- dada(fnFs.deRep, err=errFWDs, multithread=10, pool="pseudo")
dadaREVs <- dada(fnRs.deRep, err=errREVs, multithread=10, pool="pseudo")
...
saveRDS(seqtab, file = "seqtabKerguelen_16S.RDA")
write.csv(t(seqtab), "seqtab_Kerguelen_16S.csv", quote=FALSE )
...
saveRDS(seqtab.nochim, file = "seqtab.nochim.Kerguelen_16S.RDA")
write.csv(t(seqtab.nochim), "seqtab.nochim_Kerguelen_16S.csv", quote=FALSE )
...
write.csv(track, "Kerguelen_16S_DADA2_stats.csv")
...
write(asv_fasta, "Kerguelen_16S_ASVs.fasta")
